...
const gateway = createGateway({ ...config, publicOrigin: config.origin });
let stopping = false;
...
gateway.on('error', (e) => {
  console.error(`Yerel ağ geçidi açılamadı: ${e.message}`);
  stop(1);
});
gateway.listen(config.port, config.host, () =>
  console.log(
    `\nHiwell Partner Studio · ${config.origin}\nİkinci Terminal penceresinde çalıştırın:\nngrok http http://127.0.0.1:3444 --url ${config.origin}\nAna Mac açık kalmalı.\n`,
  ),
);
